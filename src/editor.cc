...
namespace kalam
{
  ...
  void Editor::PrepareBufferDrawRows(std::string &buffer) const
  {
    ...
    for (size_t row = 0; row < editor_state_.screen_rows_ - 1; ++row)
    {
      ...
      {
        size_t len = 0;
        if (editor_state_.rows_[file_row].render.size() >
            editor_state_.col_offset_)
        {
          len = editor_state_.rows_[file_row].render.size() -
                editor_state_.col_offset_;
          if (len > editor_state_.screen_cols_)
            len = editor_state_.screen_cols_;
        }
